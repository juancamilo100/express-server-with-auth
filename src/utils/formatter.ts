...
export const toLowerCaseAllPropsValues = (object: ObjectLiteral, propsToExclude?: string[]) => {
    const lowerCasedObject = object as ObjectLiteral;

    Object.keys(object).forEach((field) => {
        if (typeof lowerCasedObject[field] === "string" && !propsToExclude!.includes(field)) {
            lowerCasedObject[field] = lowerCasedObject[field].toLowerCase();
        }
    });
