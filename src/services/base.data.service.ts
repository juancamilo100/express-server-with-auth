...
class BaseDataService<T> implements IDataService<T> {
    constructor(protected entity: IGenericEntity, protected dataSource: DataSource) {}
    ...
    public async getAll(options: QueryOptions = {}) {
        try {
            const resource = await this.dataSource.getRepository(this.entity.schema).find();
            return resource as any;
        } catch (error) {
            console.error(error);
            throw new Error("Something went wrong while getting the resources");
    ...
    public async getAllByIds(ids: string[], options: QueryOptions = {}) {
        try {
            const resource = await this.dataSource.getRepository(this.entity.schema).findByIds(ids);
            return resource as any;
        } catch (error) {
            console.error(error);
            throw new Error("Something went wrong while getting the resources");
    ...
    public async create(entity: T) {
        try {
            ...
            const repo = this.dataSource.getRepository(this.entity.schema);
    ...
    public async update(id: string, entity: T) {
        try {
            ...
            const resource = await this.dataSource.getRepository(this.entity.schema)
                .createQueryBuilder()
    ...
    private buildSelectQuery(fields: object, operand: string, options: QueryOptions) {
        ...
        let query = this.dataSource.getRepository(this.entity.schema)
